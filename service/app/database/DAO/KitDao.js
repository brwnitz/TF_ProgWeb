...
const KitDao = {
    ...
    async insertKit(kitModel) {
        try{
            var connection = await connectionDB.openConnectionDB();

            var result = await connection.execute(`INSERT INTO kit (name, description, category) VALUES (?,?,?)`, [ kitModel.name, kitModel.description, kitModel.category]);
    ...
    async selectAllKit() {
        try{
            ...
            const result = await connection.promise().execute('SELECT * FROM kit');
    ...
    async selectKit(kitModel) {
        try{
            var connection = await connectionDB.openConnectionDB();

            const result = await connection.promise().execute('SELECT * FROM kit WHERE name = ?', [kitModel.name]);
    ...
    async updateKit(kitModel){
        try{
            var connection = await connectionDB.openConnectionDB();

            var result = await connection.execute(`UPDATE kit SET name = ?, description = ?, category = ? WHERE id = ?`, [kitModel.name, kitModel.description, kitModel.category, kitModel.id]);
    ...
    async deleteKit(kitModel){
        try{
            var connection = await connectionDB.openConnectionDB();

            var result = await connection.execute(`DELETE FROM kit WHERE id = ?`, [kitModel.id]);
